...
$('.arrow-icon').on('click', function() {
   ...
   if (!tableExists) {
       ...
       $.ajax({
           url: `/khs/${semester}`, // Replace with your server endpoint
           method: 'GET',
           ...
           success: function(response) {
               ...
               <div class="additional-info dark:text-white">
                   <table class="min-w-full mt-4 bg-white border dark:bg-blek-500 dark:text:white">
                       <thead>
                           <tr>
                               <th class="border py-2 w-[15%]">Kode</th>
                               <th class="border py-2 w-[40%]">Mata Kuliah</th>
                               <th class="border py-2 w-[15%]">Status</th>
                               <th class="border py-2 w-[15%]">SKS</th>
                               <th class="border py-2 w-[15%]">Nilai</th>
                           </tr>
                       </thead>
                       <tbody>`;
               ...
               response.forEach(row => {
                   ...
                   <tr class="text-center">
                       ...
                       <td class="border px-4 py-2 w-[40%]">${row.mata_kuliah}</td>
                       <td class="border px-4 py-2 w-[15%]">${row.status}</td>
                       <td class="border px-4 py-2 w-[15%]">${row.sks}</td>
                       <td class="border px-4 py-2 w-[15%]">${row.nilai}</td>
                   </tr>`;
               });
